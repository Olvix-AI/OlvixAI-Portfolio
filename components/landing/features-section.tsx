...
// Four engineers, every layer — the four layers OlvixAI covers (Homepage-Copy.md §6)
...
const features = [
  {
    ...
    description: "UI/UX from scratch, or a redesign of something people already bounce off. On PowerUp that meant rebuilding a long assessment around the fact that nobody finishes it in one sitting.",
    visual: "collab",
  },
  {
    number: "02",
    title: "Web & mobile",
    description: "Next.js on the web, React Native and Expo on mobile — one codebase to both stores, including the native health, camera and notification work that only exists on a phone.",
    visual: "deploy",
  },
  {
    number: "03",
    title: "Backend & agent logic",
    description: "NestJS and FastAPI, microservices with async workers, and the agent layer itself: tool design, orchestration, evaluation. The part most teams treat as a prompt and we treat as a system.",
    visual: "ai",
  },
  {
    number: "04",
    title: "Cloud & DevOps",
    description: "AWS, Azure, Vercel, Kubernetes. Containers, CI/CD on push, monitoring, access control, load testing, and both app store release pipelines.",
    visual: "security",
  },
];
...
// correctly rounded, so Node and the browser can disagree in the last ULP
// (36.698729810778076 vs 36.69872981077808) and React reports that as a hydration
// mismatch. 3dp is ~11 orders of magnitude clear of the divergence.
...
export function FeaturesSection() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setIsClosingVisible(true);
      },
      { threshold: 0.2 }
    );
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16 lg:mb-24">
          ...
          >
            Four engineers.
            <br />
            <span className="text-muted-foreground">Every layer.</span>
          </h2>
        </div>
        ...
        <p
          ref={closingRef}
          className={`mt-12 lg:mt-16 text-lg text-muted-foreground leading-relaxed max-w-2xl transition-all duration-700 ${
            isClosingVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
          Nobody gets handed to a subcontractor at a layer boundary, because there
          isn&apos;t one.
        </p>
      </div>
    </section>
